...
class WC_Shipping_Free_Shipping extends WC_Shipping_Method {
	...
	function init() {
		...
		if ( wmcs_get_customers_currency() == "EUR" )
			$this->min_amount = $this->get_option( 'min_amount_eur', 0 );
		else 
			$this->min_amount = $this->get_option( 'min_amount', 0 );
//echo $this->get_option( 'min_amount', 0 );
//echo wmcs_get_customers_currency();
		// Actions
	...
	function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'min_amount_eur' => array(
				'title' 		=> __( 'Minimum Order Amount in EUR', 'woocommerce' ),
				'type' 			=> 'price',
				'placeholder'	=> wc_format_localized_price( 0 ),
				'description' 	=> __( 'Users will need to spend this amount to get free shipping (if enabled above) in EUR.', 'woocommerce' ),
				'default' 		=> '0',
				'desc_tip'		=> true
			)
		);
