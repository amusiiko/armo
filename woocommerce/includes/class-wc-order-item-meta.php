...
public function display2( $flat = false, $return = false, $hideprefix = '_', $delimiter = ", \n" ) {
		...
		if ( ! empty( $formatted_meta ) ) {
			...
			foreach ( $formatted_meta as $meta_key => $meta ) {
				...
				if ( $flat ) {
					...
					$meta_list[] = '
							<span class="variation-' . sanitize_html_class( sanitize_text_field( $meta_key ) ) . '">' . wp_kses_post( $meta['label'] ) . ':' . wp_kses_post(  make_clickable( $meta['value'] )  ) . '</span>
						';
				}
			}
			...
			if ( ! empty( $meta_list ) ) {
				...
				if ( $flat ) {
					...
					$output .= '<span class="variation">' . implode( '', $meta_list ) . '</span>';
				}
			}
		}
