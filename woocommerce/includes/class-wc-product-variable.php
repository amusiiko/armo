...
//moje test
		...
		//$return_price = $t->get_variable_products_price( $this->moje, 'max', '', 'volaco' );
//echo "zaicnaem";
foreach ($this->get_children() as $child_id) {
	//pre_r($this->get_child($child_id));

			//$currency_prices = (array)get_post_meta( $child_id, 'wmcs_currency_prices', TRUE );
			//print_r($currency_prices);echo "WHATTT<hr>";
}
		


		//echo wmcs_get_customers_currency()."########SS@@@";
