...
jQuery(function(a) {
  ...
  var b = {
      ...
      if (c.triggerHandler("checkout_place_order")!==!1 && c.triggerHandler("checkout_place_order_" + a("#order_review input[name=payment_method]:checked").val())!==!1) {
        ...
        1 != d["blockUI.isBlocked"] && c.block({
          ...
          success: function(b) {
            var d = "";
            try {
              if (b.indexOf("<!--WC_START-->") >= 0 && (b = b.split("<!--WC_START-->")[1]), b.indexOf("<!--WC_END-->") >= 0 && (b = b.split("<!--WC_END-->")[0]), d = a.parseJSON(b), "success" !== d.result)
    ...
    init: function() {
      //a("body").on("click", "a.showcoupon", this.show_coupon_form), a("body").on("click", ".woocommerce-remove-coupon", this.remove_coupon),
       a("form.checkout_coupon").submit(this.submit)
    },
    ...
    submit: function() {
      ...
      return a.ajax({
        ...
        data: c,
        success: function(c) {
          a(".woocommerce-error, .woocommerce-message").remove(), b.removeClass("processing").unblock(), c && (b.before(c),//b.slideUp(),
